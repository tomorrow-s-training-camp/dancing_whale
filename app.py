...
# 마이페이지 - 페이지의 게시글 클릭시 id와 content 비교 후 게시글 내용 출력
def mycontent(myid):
    if 'username' in session:
        username = session['username']
...
@app.route('/mypage/edit/<user_name>', methods=['GET', 'POST'])
# GET -> 유지되고있는 username 세션과 현재 접속되어진 id와 일치시 edit페이지 연결
# POST -> 접속되어진 id와 일치하는 title, content를 찾아 UPDATE
def mycontentedit(user_name):
    if request.method == 'POST':
        if 'username' in session:
            username = session['username']
            ...
            conn = connectsql()
            cursor = conn.cursor()
            query = "UPDATE user SET user_name = %s, user_intro = %s WHERE user_name = %s"

            value = (myusername, myuserintro, user_name)
            cursor.execute(query, value)
            # query = f"SELECT user_name, user_intro FROM user WHERE user_name = '{user_name}'"
            # cursor.execute(query)
            ...
            cursor.close()
            conn.close()
            return redirect(f'/mypage/{user_name}')
            # return render_template('mypageEdit.html', mydataedit=mycontentedit)
    else:
        if 'username' in session:
            username = session['username']
            conn = connectsql()
            cursor = conn.cursor()
            query = f"SELECT user_name, user_intro FROM user WHERE user_name = '{user_name}'" # where user_id로 잘못써서 mysql에서 데이터 안나옴
            cursor.execute(query)
